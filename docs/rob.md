...
提交候选与实际提交在不同组合逻辑中计算，使候选元数据不依赖 `commit_ready_i`，避免 LSQ 提交回压形成组合环。默认 LSQ tag 宽度为 5 位。store 已完成且尚无异常时，ROB 还可接受一次同 tag 的异常完成，用来记录缓存写响应返回的晚到错误；异常到达前 LSQ 保持该 store 的提交回压。
